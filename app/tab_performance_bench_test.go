...
func BenchmarkDebouncedVsImmediateUpdates(b *testing.B) {
	b.Run("WithDebouncing", func(b *testing.B) {
		...
		h.selectionUpdateDelay = 100 * time.Millisecond // Normal debouncing
		...
		for i := 0; i < b.N; i++ {
			...
			for j := 0; j < 10; j++ {
				...
				h.fastInstanceChanged() // This should be lightweight
			}
		}
	})
	...
	b.Run("WithoutDebouncing", func(b *testing.B) {
		...
		for i := 0; i < b.N; i++ {
			...
			for j := 0; j < 10; j++ {
				h.list.SetSelectedIdx(j % 25)
				h.instanceExpensiveUpdate() // This runs every time
			}
		}
	})
}
...
func setupRealisticBenchmarkHome(b *testing.B, sessionCount int) *home {
	...
	h := &home{
		...
		state:                stateDefault,
		appState:             appState,
		selectionUpdateDelay: 150 * time.Millisecond, // Realistic debounce timing
	}
